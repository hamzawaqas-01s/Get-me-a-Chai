...
const Username = ({ params }) => {
    ...
    const handleDonate = async () => {
        ...
        if (data.url) window.location.href = data.url; // send donor to Stripe
        setLoading(false);
    };
                    ...
                    <div className="makePayment w-full md:w-1/2 bg-[#FBF8F1] border border-[#DDD2BE] rounded-lg px-2 md:p-10">
                        ...
                        <div className='flex gap-2 flex-col'>
                            ...
                            <div>
                                ...
                                <input value={name} onChange={(e) => setName(e.target.value)}  type="text" className='w-full p-3 rounded-lg bg-[#FBF8F1] border border-[#DDD2BE]' placeholder='Enter Name' />
                            </div>
                            <input value={message} onChange={(e) => setMessage(e.target.value)} type="text" className='w-full p-3 rounded-lg bg-[#FBF8F1] border border-[#DDD2BE]' placeholder='Enter Message' />
                        ...
                        <div className='flex flex-col md:flex-row gap-2 mt-5'>
                            <button className='bg-[#FBF8F1] border border-[#DDD2BE] p-3 rounded-lg' onClick={() => setAmount(10)}>Pay $10</button>
                            <button className='bg-[#FBF8F1] border border-[#DDD2BE] p-3 rounded-lg' onClick={() => setAmount(20)}>Pay $20</button>
                            <button className='bg-[#FBF8F1] border border-[#DDD2BE] p-3 rounded-lg' onClick={() => setAmount(30)}>Pay $30</button>
                        </div>
                    </div>
                </div>
            </div>
